...
const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  ...
  return (
    <section className="min-h-screen w-full bg-black relative overflow-hidden flex flex-col justify-start items-center pt-28 md:pt-40 pb-10">
      {/* FONDO */}
      ...
      {/* LOGO HOME */}
      <div className="z-10 w-full flex justify-center mb-8 md:mb-12 -mt-6 md:mt-0 px-4">
      ...
      <div className="z-20 flex flex-col items-center justify-center gap-8 w-full px-4">
        
        {/* REEL (Top) */}
        <div onClick={() => setShowVideo(true)} className="relative w-full max-w-[320px] aspect-video rounded-xl overflow-hidden cursor-pointer shadow-2xl border border-white/10 hover:border-bocha-pink/50 transition-all hover:scale-105 group">
            ...
            <div className="absolute inset-0 bg-black/30 flex items-center justify-center group-hover:bg-black/20 transition-all">
                <div className="w-12 h-12 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center border border-white/30 group-hover:scale-110 transition-transform"><svg className="w-5 h-5 fill-white ml-0.5" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg></div>
            </div>
        </div>
